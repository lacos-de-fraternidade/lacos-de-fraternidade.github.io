...
O botão e o link alternativo devem usar exatamente:

`{{ .ConfirmationURL }}`

Não substitua por URL fixa.
...
4. A página aberta deve ser `/area-restrita/ativar/`, não `localhost`, quando o envio partiu do site publicado.
5. Repetir com **Esqueci minha senha** e o template de recuperação.

## Checklist

- [ ] Assunto do convite em português
- [ ] Conteúdo sem inglês padrão do Supabase
- [ ] `{{ .ConfirmationURL }}` no botão e no link alternativo
